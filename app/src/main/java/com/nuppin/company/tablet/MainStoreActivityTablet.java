...
public class MainStoreActivityTablet extends AppCompatActivity implements
        ...
        FrOrdersTablet.ToActivity,
        FrAgeTablet.ToActivity,
    ...
    private void openFragment(Fragment fragment, String tagBackStack, boolean tag) {
        ...
        if (tagBackStack.equals("frListaProdutosLoja") || tagBackStack.equals("FrFuncionarios") || tagBackStack.equals("FaturasFr") || tagBackStack.equals("FrCupons") || tagBackStack.equals("FrRelatorioStore") || tagBackStack.equals("FrFinanceiroStore") || tagBackStack.equals("ListaServicos") || tagBackStack.equals("MainFr") || tagBackStack.equals("rating") || tagBackStack.equals("StoreHorariosToolbar") || tagBackStack.equals("FrCarrinhoManual") || tagBackStack.equals("ListaServicosManual")) {
            clearBackStack();
    ...
    public void ToolbarClick(int id, Company company) {
        switch (id) {
            ...
            case R.id.onlineOrNot:
                openFragment(StoreHorarios.newInstance(company), "StoreHorariosToolbar", true);
                break;
            case R.id.invisible:
                if (company.getCategory_company_id().equals("3")) {
                    openFragment(ConfigServico.newInstance(company), "configServico", true);
    ...
    public void onClickStatus(String index, String stoId) {
        openFragment(FrOrdersTablet.newInstance(stoId, index), R.id.containerB, "FrOrders", true);
        containerB.setVisibility(View.VISIBLE);
    }

    @Override
    public void onClickStatusAgendamento(String index, String stoId) {
        openFragment(FrAgeTablet.newInstance(stoId, index), R.id.containerB, "FrAge", true);
        containerB.setVisibility(View.VISIBLE);
    }
    ...
    public void chat(Scheduling agendamento) {
        openFragment(FrChat.newInstance(agendamento.getUser_id(), agendamento.getScheduling_id(), agendamento.getCompany_id(), agendamento.getUser_name()), AppConstants.CHAT, true);
    }
    ...
    public void onBackStackChanged() {
        if (containerB.getChildCount() < 1 && backCount > getSupportFragmentManager().getBackStackEntryCount()) {
            containerB.setVisibility(View.GONE);
        }
    ...
    private void clearLastBackStack(String tag) {
        FragmentManager manager = getSupportFragmentManager();
        if (containerB.getChildCount() > 0) {
            manager.popBackStack(tag, FragmentManager.POP_BACK_STACK_INCLUSIVE);
        }
    }

    private void openFragment(Fragment fragment, int container, String tagBackStack, boolean tag) {
        clearLastBackStack(tagBackStack);
        getSupportFragmentManager().addOnBackStackChangedListener(this);
        transaction = getSupportFragmentManager().beginTransaction();
        transaction.replace(container, fragment, tagBackStack);
        if (tag) {
            transaction.addToBackStack(tagBackStack);
        }
        transaction.commit();
